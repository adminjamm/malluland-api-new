...
export class ChatsService {
  ...
  async listRooms(
    userId: string,
    page: number
  ): Promise<{ items: ChatRoomListItem[]; total: number; pageSize: number }> {
    const limit = 20;
    const offset = (page - 1) * limit;
    const { items, total } = await (this.repo as any).listRoomsForUserWithTotal(
      userId,
      limit,
      offset
    );
  ...
  async listRoomsV2(
    ...
    page: number,
    pageSize = 20
  ): Promise<{ items: any[]; total: number; pageSize: number }> {
    const limit = pageSize;
    const offset = (page - 1) * limit;
    const { items, total } = await (this.repo as any).listRoomsV2WithTotal(
      userId,
      status,
      limit,
      offset
  ...
  async sendMessage(chatId: string, senderUserId: string, text: string) {
    ...
    const isParticipant = await this.chatRepo.isActiveParticipant(
      chatId,
      senderUserId
    );
    if (!isParticipant)
    ...
    const dbRows = await this.chatRepo.createTextMessage({
      chatId,
      senderUserId,
      body,
    });
  ...
  async archiveChatRoom(
    ...
    const rows = await this.repo.archiveChatRoom(userId, chatId);
    if (!rows) return null;
    return { rows };
  }
  async ArchivedListTotal(
    userId: string
  ): Promise<{ data: { total: number } }> {
    const { total } = await (this.repo as any).archivedListRoomsTotal(userId);
    return { data: total };
  }
}
