...
export class KrakenHelper {
  ...
  // Asynchronous optimization: returns the Kraken job id (similar to sendForOptimization)
  async sendForOptimization(imageUrl: string): Promise<string | null> {
    ...
    try {
      ...
      // Kraken async URL returns an id we can store
      return data?.id ?? null;
    } catch (err) {
  ...
  // Synchronous optimization with small-file short-circuit (similar to sendForOptimizationSync)
  async sendForOptimizationSync(imageUrl: string): Promise<KrakenOptimizeResponse> {
    ensureKrakenCreds();

    // HEAD check to skip tiny files (<= 400KB)
    try {
