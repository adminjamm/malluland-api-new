...
async function main() {
  ...
  const existingUserStates = await db
    .select({ name: userStates.name })
    .from(userStates);
  const existingUserStateNames = new Set(existingUserStates.map((s) => s.name));
  const newUserStates = userStatesData.filter(
    (s) => !existingUserStateNames.has(s.name)
  );
  if (newUserStates.length) {
    await db.insert(userStates).values(newUserStates);
  }
  ...
  const actresses = [
    { id: 1, name: "Manju Warrier", slug: "manju-warrier", isActive: true },
    {
      ...
      slug: "parvathy-thiruvothu",
      isActive: true,
    },
  ];
  const existingActresses = await db
    .select({ id: catalogActresses.id })
  ...
  const newCurrencies = currencyData.filter(
    (c) => !existingCurrencyCodes.has(c.code)
  );
  ...
  const reportOptions = [
    {
      id: randomUUID(),
      optionText: "Spam or Scam",
      displayOrder: 1,
      isActive: true,
    },
    {
      id: randomUUID(),
      optionText: "Harassment or Hate",
      displayOrder: 2,
      isActive: true,
    },
    {
      id: randomUUID(),
      optionText: "Inappropriate Content",
      displayOrder: 3,
      isActive: true,
    },
    {
      id: randomUUID(),
      optionText: "Fake Profile",
      displayOrder: 4,
      isActive: true,
    },
  ...
  const existingOptions = await db
    .select({ optionText: blockAndReport.optionText })
    .from(blockAndReport);
  const existingOptionTexts = new Set(existingOptions.map((o) => o.optionText));
  const newOptions = reportOptions.filter(
    (o) => !existingOptionTexts.has(o.optionText)
  );
  ...
  const badgeCap = (await db
    .select()
    .from(appSettings)
    .where(eq(appSettings.key, "badge_cap"))
    .limit(1)) as any;
  if (!badgeCap.length) {
    await db
      .insert(appSettings)
